...
def steepest_descent(f, g, x0, iterations, error):
  ...
  for i in range(iterations):
    ...
    x = x + alpha * pk
    if i % 10 == 0:
      print("  iter={}, g={}, alpha={}, x={}, f(x)={}".format(i, pk, alpha, x, f(x)))
...
def top_cumulative_g_diff(cumulative_y, M):
...
def PQN(f, g, x0, iterations, error, portion):
    ...
    for i in range(iterations):
        ...
        if i != 0 and i % k == 0:
            Hk = I        
            p_index, np_index = top_cumulative_g_diff(cumulative_y, M)
            cumulative_y = np.zeros(N)
        ...
        if i % 10 == 0:
            print("  iter={}, direction={}, alpha={}, x={}, f(x)={}, SquaredGradient={}".format(i, pk, alpha, xk, f(xk), np.linalg.norm(gk)))
            #print("  iter={}, x={}, f(x)={}".format(i, xk, f(xk)))
